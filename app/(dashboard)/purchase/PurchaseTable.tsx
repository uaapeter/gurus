...
import { selectSelectedSupplier } from '@/app/reducers/supplierReducer'
...
function PosTable({token, discounts, pendingsales}: { discounts:any[], pendingsales:any[], token:string}) {
    ...
    const supplier:any = useSelector(selectSelectedSupplier)
    ...
    const handleAddToCart = () => {
        if(product) {
            const { _id, productBatch, productName, cartonQty, costPrice, barcode } = product
    ...
    useMemo(() =>{
     ...
     useEffect(() => {
        dispatch(setOrderType('PURCHASE'))
     }, [])
